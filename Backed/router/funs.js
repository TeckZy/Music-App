...
module.exports = {
  login: function (socket, db, email, password, Device, ipAddress) {
    ...
    var source = "Android";
    var query1 =
      "select * from log_in WHERE Email='" +
      email +
      "' and Password='" +
      password +
      "'";
    ...
    db.query(query1, function (err, result) {
      ...
      if (result.length > 0) {
        ...
        if (res === undefined) {
          ...
          var query =
            "update log_in set LastLoginDateTime='" +
            datetime +
            "',CurrentStatus='" +
            st +
            "',CurrentLoginFrom='" +
          ...
          db.query(query, function (err, result) {
            ...
            if (result.affectedRows > 0) {
              ...
              module.exports
                .getFriends(socket, db, idsforimg, roomstojoin)
                .then(function () {
                  db.release();
  ...
  getFriends: function (socket, db, idsforimg, roomstojoin) {
    return new Promise(function (resolve, reject) {
      query1 =
        "SELECT * FROM friendships INNER JOIN log_in ON friendships.UseridTo = log_in.Id WHERE friendships.UseridFrom = '" +
        socket.userid +
        "'";
      query =
        "SELECT * FROM friendships INNER JOIN log_in ON friendships.UseridFrom = log_in.Id WHERE friendships.UseridTo = '" +
        socket.userid +
        "' UNION " +
        query1 +
        "";
      db.query(query, function (err, result) {
        ...
        if (result.length > 0) {
          ...
          console.log("No friends found online");
        }
      });
  ...
  registration: function (
    ...
    db.beginTransaction(function (err) {
      ...
      var query1 =
        "insert into log_in(Name,Mobile,Email,Password,LastLoginDateTime,LastLogoutDateTime,LoginCount,CurrentStatus,CurrentLoginFrom) values('" +
        username +
        "','" +
        mobile +
        "','" +
      ...
      db.query(query1, function (error, results, fields) {
        ...
        var query2 =
          "insert into user_accounts(Name,Gender,Mobile,Email,Password,Date_Time,Id) values('" +
          username +
          "','" +
          gender +
          "','" +
  ...
  DP_QUERY: function (socket, idsforimg) {
    ...
    Promise.each(idsforimg, function (item) {
      if (fs.existsSync("../users/" + item + "/USER_DP.jpg")) {
        return fs
          .readFileAsync("../users/" + item + "/USER_DP.jpg")
          .then(function (val) {
  ...
  CURRENT_SONG_UPDATE: function (
    ...
    db.query(query, function (err, result) {
      ...
      if (result.length > 0) {
        ...
        query =
          "select times from history Where Song_name = '" +
          songName +
          "' AND  Song_artist = '" +
          songArtist +
          "' AND Song_album = '" +
        ...
        db.query(query, function (error, result) {
          ...
          db.beginTransaction(function (err) {
            ...
            if (result.length > 0) {
              ...
              query1 =
                "insert into currents(UserID,User_name,Song_name,Song_album,Song_artist,User_Email,song_duration,Song_progress,Date,Time) values('" +
                userid +
                "','" +
                name +
                "','" +
              ...
              query1 =
                ...
                "')";
              query2 =
                "insert into history(UserID,User_name,Song_name,Song_album,Song_artist,User_Email,song_duration,Song_progress,Date,Time,times) values('" +
                userid +
                "','" +
                name +
                "','" +
            ...
            db.query(
              "DELETE FROM currents WHERE UserID=" +
                userid +
                " AND User_Email = '" +
                Email +
                "'",
    ...
    if (invite == true) {
      ...
      db.query(query, function (err, result) {
        ...
        if (result.length > 0) {
          ...
          var query =
            "insert into friendships(UseridFrom,UseridTo,Date,Time,Accept,Reject) values('" +
            userid +
            "','" +
            friend_id +
            "','" +
    ...
    if (del == true) {
      ...
      db.query(query, function (err, result) {
        ...
        if (result.length > 0) {
          ...
          console.log("\n delete -->" + friend_id);
          var query =
            "DELETE FROM friendships Where (UseridFrom =" +
            userid +
            " AND UseridTo = " +
            friend_id +
            ") OR( UseridFrom = " +
  ...
  PROGRESS_UPDATER: function (socket, db, userid, progress) {
    query =
      "update currents set Song_progress = " +
      progress +
      " WHERE UserID = " +
      userid +
      "";
  ...
  GET_USER: function (socket, db, userid, hist) {
    function getUser() {
      return new Promise(function (resolve, reject) {
        ...
        query =
          "SELECT * FROM currents INNER JOIN (SELECT Id,Mobile from log_in WHERE Id = 1 ) b ON currents.UserID = b.Id WHERE UserID = " +
          userid +
          "";
      ...
      if (clients !== undefined) {
        ...
        Promise.each(clients, function (s) {
          ...
          if (users && users.length) {
            var query =
              "SELECT * FROM currents WHERE UserID IN (" +
              users.toString() +
              ")";
  ...
  UPLOAD_DP: function (socket, userid, buffer) {
    ...
    fs.writeFile(
      __dirname + "/users/" + socket.userid + "/USER_DP.jpg",
      buffer,
      { flags: "wx" },
      function (err) {
        if (err) throw err;
        ...
        fs.readFile(
          ...
          function (err, data) {
            ...
            response = {
              error: "false",
              dp: arrByte,
              message: "Display Pic Saved Successfully",
            };
...
function logout(socket) {
  ...
  io.of("/").adapter.clientRooms(socket.id, (err, rooms) => {
    ...
    Promise.each(rooms, function (item) {
      socket.leave(item);
      console.log("Client has been Removed from Every Rooms");
    })
      .then(function () {
        io.in(socket.userid).clients((err, clients) => {
  ...
  db.query(query, function (err, result) {
    ...
    if (result.length > 0) {
      ...
      var response = {
        error: "false",
        packet: curr_packet,
        message: "Music Changed Successfully",
      };
